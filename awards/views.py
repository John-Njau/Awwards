...
def project_details(request, id):
    ...
    # fetch average rating for all reviews
    avg_usability=Reviews.objects.filter(project=project).aggregate(avg_rating=Avg('usability_rating').round(2))
    avg_content=Reviews.objects.filter(project=project).aggregate(avg_rating=Avg('content_rating').round(2))
    avg_design=Reviews.objects.filter(project=project).aggregate(avg_rating=Avg('design_rating').round(2))
